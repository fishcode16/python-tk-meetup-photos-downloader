...
def init_app():
    ...
    if not os.path.exists(CONFIG_FILE):
        default_config = {}
        default_config["iptc_tag"] = False
        default_config["mtime_tag"] = True
        default_config["photo_list_tag"] = True
    ...
    # Load config json
    # (FUTURE) Check for invalid config file
    config_data = json.loads((open(CONFIG_FILE).read()))

    #config_json["iptc_tag"]
    #config_json["mtime_tag"]
    #config_json["photo_list_tag"]

    #    for config_item in ("iptc_tag", "mtime_tag", "photo_list_tag"):
    #        print(config_item)
...
def display_group_frame(force_update):
...
def group_clicked(event):
    ...
    if item_id:  # only do something, if there is an item selected
        ...
        if Group_idx != g_idx:
            ...
            display_event_frame(0)  # update the event frame
...
def display_event_frame(force_update):
...
def display_album_frame(force_update):
...
def update_album_frame():
    ...
    # enable the selection buttons, if there are files to be downloaded
    if need_dl_count:
...
def photo_clicked(event):
    ...
    album_download_btn.state(['disabled'])  # disable first

    # If there is selection
    if photo_tree.selection():
        # Scan through selection. Enable the download button if there is at
        # least one photo is eligible for download
...
def album_select_all(yes):
    ...
    if yes:  # select all items
        ...
        # disable the download button, since nothing was selected
        album_download_btn.state(['disabled'])
...
album_none_btn = ttk.Button(album_frame,
                            text='Clear all',
                            command=lambda: album_select_all(0))
...
if headers == '':
    # if no access token, then display an error message
    mesg_box(
        'error',
